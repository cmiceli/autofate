...
trait FateRunner {
    ...
    fn run_test(&self, commit_hash: git2::Oid) -> Result<bool, std::io::Error>;
}

struct SerialFateRunner {
    config: Yaml,
}

impl SerialFateRunner {

    fn collect_results(&self, base_repo: &str) -> Result<HashMap<String, FateResult>, std::io::Error> {
        ...
        for entry in walker.filter_entry(|e| util::is_report_file(e)) {
            ...
            match entry.unwrap().file_name().to_str() {
                ...
                Some(s) => results.insert(util::to_test_name(s), FateResult{report: Some(std::fs::read_to_string(format!("{}{}",&report_path, s))?), error: None}),
            };
        }
        info!("Collecting error results from {}", &report_path);
        let walker = walkdir::WalkDir::new(&report_path).contents_first(true).into_iter();
        for entry in walker.filter_entry(|e| util::is_err_file(e)) {
            match entry.unwrap().file_name().to_str() {
                None => return Err(Error::new(ErrorKind::UnexpectedEof, "failed to resolve a report file to string")),
                Some(s) => {
                    let error_file = std::fs::read_to_string(format!("{}{}",&report_path, s))?;
                    results.entry(util::to_test_name(s)).or_insert(FateResult{report: None, error: None}).error = Some(error_file);
    ...
    fn submit_results(&self, fate_results: &HashMap<String, FateResult>, commit_hash: git2::Oid) -> Result<(), std::io::Error> {
        ...
        let result_file = format!("{}/{}", self.config["fate"]["result_directory"].as_str().unwrap(), commit_hash);
        info!("writing report to {}", &result_file);
        std::fs::write(&result_file, resulting_string)
    }

    fn run_commands(&self, run_dir: &str, commit_hash: &git2::Oid) -> Result<(), std::io::Error> {
        let cmd_args: Vec<&str> = self.config["fate"]["command"].as_str().unwrap().split(" ").collect();
        info!("Running pre-run command: {}", self.config["fate"]["pre_run_command"].as_str().unwrap());
        let prerun: Vec<&str> = self.config["fate"]["pre_run_command"].as_str().unwrap().split(" ").collect();
        let mut prerun_cmd = Command::new(&prerun[0]);
        if prerun.len() > 1 {
            prerun_cmd.args(&prerun[1..]);
        }
        let res = prerun_cmd.current_dir(run_dir).output()?;
        let pre_run_output = format!("{}/{}.prerun.stdout", self.config["fate"]["result_directory"].as_str().unwrap(), commit_hash);
        std::fs::write(pre_run_output, res.stdout)?;
        let pre_run_output = format!("{}/{}.prerun.stderr", self.config["fate"]["result_directory"].as_str().unwrap(), commit_hash);
        ...
        info!("Running build/test command: {}", self.config["fate"]["command"].as_str().unwrap());
        let res = Command::new(cmd_args[0])
            .args(&cmd_args[1..])
            .env("FATE_SAMPLES", self.config["fate"]["samples_directory"].as_str().unwrap())
            .current_dir(run_dir)
            .output()
            .unwrap();
        let cmd_output = format!("{}/{}.cmd.stdout", self.config["fate"]["result_directory"].as_str().unwrap(), commit_hash);
        std::fs::write(cmd_output, res.stdout)?;
        let cmd_output = format!("{}/{}.cmd.stderr", self.config["fate"]["result_directory"].as_str().unwrap(), commit_hash);
...
impl FateRunner for SerialFateRunner {
    fn new(config: Yaml) -> SerialFateRunner {
        SerialFateRunner{config: config}
    }

    fn run_test(&self, commit_hash: git2::Oid) -> Result<bool, std::io::Error> {
        let base_repo = self.config["repo"]["directory"].as_str().unwrap();
        let new_repo_loc = format!("{}/{}", self.config["fate"]["tmp_directory"].as_str().unwrap(),commit_hash);
        ...
        let res = self.run_commands(&new_repo_loc, &commit_hash);
        ...
        let fate_results = self.collect_results(&new_repo_loc)?;
        info!("submitting results");
        self.submit_results(&fate_results, commit_hash)?;
...
pub fn run(config: Yaml) {
    ...
    let sleep_time = std::time::Duration::from_secs(config["fate"]["run_interval_sec"].as_i64().unwrap().try_into().unwrap());
    let runner = SerialFateRunner::new(config);
    loop {
        ...
        // this starts at origin/master and walks backwards in time.
        ...
        for unseen_commit in unseen_commits.iter() {
            ...
            match runner.run_test(*unseen_commit) {
                Ok(_) =>  {
